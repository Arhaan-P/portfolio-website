  ...
  return (
    <section className="mx-auto max-w-5xl px-4 py-20 sm:px-6 sm:py-28 relative">
      ...
      <RevealGroup className="mt-10">
        <motion.div layout={!shouldReduceMotion} className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          <AnimatePresence mode="popLayout" initial={false}>
            {visible.map((project) => (
              <motion.div
                key={project.slug}
                layout={!shouldReduceMotion}
                initial={shouldReduceMotion ? { opacity: 0 } : { opacity: 0, scale: 0.9, rotateX: -30, y: 50, z: -50 }}
                animate={shouldReduceMotion ? { opacity: 1 } : { opacity: 1, scale: 1, rotateX: 0, y: 0, z: 0 }}
                exit={shouldReduceMotion ? { opacity: 0 } : { opacity: 0, scale: 0.9, rotateX: 30, y: -50, z: 50 }}
                transition={shouldReduceMotion ? { duration: 0 } : { duration: 0.4, type: "spring", bounce: 0.3 }}
